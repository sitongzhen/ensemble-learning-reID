...
class TrainSet(Dataset):
  ...
  def __init__(self,
               ...
               ids2labels=None,
               **kwargs):
    super(TrainSet, self).__init__(dataset_size=int(len(im_names)*sample_ratio), **kwargs)
  ...
  def next0_batch(self):
    ...
    """
    if self.epoch0_done and self.shuffle:
      self.prng.shuffle(self.im_names)
    samples, self.epoch0_done = self.prefetcher.next_batch()
    ...
    return ims, im_names, labels, mirrored, self.epoch0_done
